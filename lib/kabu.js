// 株信用チェッカー: データ取得ロジック (ローカルサーバー / サーバーレス API 共通)
//
...
// 取得結果のキャッシュ (信用残は週次更新なので 10 分保持で十分)
const cache = new Map();
...
async function getMargin(code) {
  ...
  if (!ok) {
    return { error: `銘柄コード「${code}」のページを取得できませんでした (HTTP ${status})` };
  }
...
async function searchStocks(q) {
  ...
  if (!ok) return { error: `検索に失敗しました (HTTP ${status})` };
